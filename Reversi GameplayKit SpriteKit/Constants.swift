//
//  Constants.swift
//  Try4
//
//  Created by Владислав Эдуардович Дембский on 02.02.16.
//  Copyright © 2016 Vladislav Dembskiy. All rights reserved.
//
// Struct to keep all game constants
//

import Foundation

struct Constants {
    static let cellBackgroundImage = "BoardFabric"

    static let cellImage = "CellImage"
    
    struct ChipImages {
        static let whiteChip = "WhiteChip"
        static let blackChip = "BlackChip"
    }
    struct Fonts {
        static let countsFont = "Chalkboard SE Regular"
        static let alertFont = "Chalkboard SE Regular"
    }
    static let countsLabelSpriteName = "CountsLabel"
    static let alertSpriteName = "Alert"

    static let chipUserDataKey = "Color"
}
